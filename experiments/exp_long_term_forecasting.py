...
from experiments.exp_basic import Exp_Basic_Fixed
...
class Exp_Long_Term_Forecast_Fixed(Exp_Basic_Fixed):
    """
    长期预测实验类（修复版）

    修复内容：
    - predict() 方法增加 PEMS/Solar 数据集的 batch_x_mark / batch_y_mark None 处理
    - 继承自 Exp_Basic_Fixed，使用修复后的模型注册表
    """
    ...
    def _build_model(self):
        ...
        if isinstance(pretrained_path, str) and pretrained_path.strip():
            ...
            if os.path.exists(pretrained_path):
                ...
                print(f'Pretrained load summary - missing keys: {len(missing_keys)}, unexpected keys: {len(unexpected_keys)}')
            else:
                print(f'Warning: pretrained_path not found, skip loading: {pretrained_path}')
    ...
    def train(self, setting):
        ...
        #     print('using type1 lradj')
        #     scheduler = optim.lr_scheduler.ExponentialLR(model_optim, gamma=0.5)
    ...
    def predict(self, setting, load=False):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle) in enumerate(pred_loader):
                ...
                # 【修复】增加 PEMS/Solar 数据集的 mark None 处理
                # 原版缺少此判断，对 PEMS/Solar 数据集会导致 AttributeError
                if 'PEMS' in self.args.data or 'Solar' in self.args.data:
                    batch_x_mark = None
                    batch_y_mark = None
                else:
