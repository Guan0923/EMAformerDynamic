"""
Dynamic Embedding Layer - 数据驱动的动态统计特征嵌入
...
class StatisticalFeatures(nn.Module):
    ...
    def forward(self, x):
        ...
        # 2. 频域特征 (2*fft_k 维)
        # TODO : 可以考虑增加频域特征的多样性，例如引入频带统计、频谱熵等，以增强对不同类型周期性的捕捉能力。
        # TODO : 递归频域增强的层级决策可以进一步细化，例如引入更多层级、动态调整每层保留的频点数，甚至引入注意力机制来聚焦重要频段。
        # TODO : 频域特征的解码策略可以更加灵活，例如引入可学习的频率选择器，或者结合频带统计与 top-k 选择，以适应不同数据集的频谱特性。
        # 目前实现了三种频域特征解码策略：硬选择 top-k、软选择 top-k 和多带宽统计后投影。未来可以考虑引入更多创新的频域特征提取方法，以进一步提升模型对复杂周期性模式的捕捉能力。
        # 频域特征提取的创新点在于引入了递归频域增强机制，通过多层次的频谱分析逐步提取主导频率成分，并通过可学习的深度决策动态调整分析深度。这种机制使得模型能够更灵活地适应不同数据集的频谱特性，尤其是在存在多个周期性模式或高频残差时表现出更强的捕捉能力。此外，提供了多种频域特征解码策略，允许模型根据数据特性选择最合适的频域表示方式，从而进一步提升对复杂周期性模式的建模能力。
        # TODO 现在缺陷在于只能返回 2 * fft_k 维的频域特征，未来可以考虑引入更多样化的频域特征，例如频带统计、频谱熵等，以增强对不同类型周期性的捕捉能力。
...
class DynamicJointEmbedding(nn.Module):
    ...
    def forward(self, stats, batch_size, n_channels, phase_indices, device):
        ...
        """
        # 统计特征投影
        # TODO 能不能通过[channel_emb = self.stat_proj(stats)  # (B, N, d_model)]的结果再接一个线性映射来表示
        ...
        t = phase * 2 * 3.14159265
        sin_t = torch.sin(t).unsqueeze(-1)
        cos_t = torch.cos(t).unsqueeze(-1)
        normalized_t = phase.unsqueeze(-1)

        # TODO 能不能通过[phase_emb = self.phase_embed(B, N, phase_indices, x.device)]的结果再接一个线性映射来表示，
        # 但是会导致[语义上不完全等价]
        phase_input = torch.cat([sin_t, cos_t, normalized_t], dim=-1)
